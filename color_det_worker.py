...
class SmokeDetectorWorker(BaseWorker):
    ...
    def detect_smoke_in_roi(self, frame, roi_bbox, roi_key):
        ...
        # 2️⃣ Color deviation (LAB color space)
        ...
        # 3️⃣ Texture softness (blurriness)
        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        blur_score = cv2.Laplacian(gray, cv2.CV_64F).var()
        blur_mask = (blur_score < self.blur_threshold).astype(np.uint8) * 255

        # 4️⃣ Combine masks (motion + color deviation + texture)
