...
class SmokeDetectorWorker(BaseWorker):
    ...
    def detect_smoke_in_roi(self, frame, roi_bbox, roi_key):
        ...
        if fgmask is None:
            log(f"[{self.name}] Warning: fgmask is None for ROI {roi_key}")
            return False, None
        ...
        L, A, B = cv2.split(lab)
        color_var = cv2.addWeighted(
            cv2.absdiff(A, np.median(A).astype(np.uint8)), 0.5,
            cv2.absdiff(B, np.median(B).astype(np.uint8)), 0.5, 0
        )
        ...
        # 4️⃣ Ensure all masks are uint8 and same size
        for mask_name, mask in zip(["fgmask", "color_mask", "blur_mask"], [fgmask, color_mask, blur_mask]):
            if mask is None or mask.shape[:2] != gray.shape:
                log(f"[{self.name}] Skipping ROI {roi_key}: Invalid mask {mask_name}")
                return False, None
        ...
        # ✅ Define ratio here (proportion of "smoke-like" pixels)
        ratio = np.sum(combined > 0) / (roi.shape[0] * roi.shape[1])

        # 6️⃣ Directional motion (optical flow)
        upward_ratio = 0.0
        if roi_key in self.prev_gray_map:
            try:
                ...
                log(f"[{self.name}] Optical flow error for ROI {roi_key}: {e}")
                upward_ratio = 0.0
        ...
        log(f"[{self.name}] ROI {roi_key}: motion={motion_ratio:.3f}, ratio={ratio:.3f}, upward={upward_ratio:.3f}, blur={blur_score:.1f}, detected={detected}")
